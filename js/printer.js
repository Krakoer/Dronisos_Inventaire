...
//Print from the addentry form with the Ajouter et Imprimer button
function printFromAdd(formType){
    var ser, model, id
    var form = document.getElementById(formType+"Form") // Get the form
    var inputs = form.getElementsByTagName("input"); // Get the inputs
    for(var input of inputs){
        if(input.name == "Reference")
            ser = input.value
        else if(input.name == "Famille")
            model = input.value
        else if(input.name == "Ref2")
            id = input.value
    }
    printLabel(ser, model, id)
}

// Print the last row or the given data
function printLabel(Serial, Modele, ID) {
    var data = {}
    var labelType
    if (arguments.length > 0) {
        ...
        data["Modèle"] = Modele
        data["ID Dronisos"] = ID
        labelType = document.getElementById("EntryLabelTypeSelector").value;
    }
    else {
        data = selectLastRow();
        labelType = document.getElementById("labelTypeSelector").value; // Select the type of label
    }
        var labelString = "";
        switch (labelType) {// Build the label
            case "zephyr":
                labelString = buildLabel(labelType, data["Modèle"], data["N° Série"]);
                break;
            case "helios":
                labelString = buildLabel(labelType, data["N° Série"]);
                break;
            case "materiel":
                labelString = buildLabel(labelType, data["Modèle"], data["ID Dronisos"], data["N° Série"])
                break;
            case 'bureautique':
                labelString = buildLabel(labelType, data["Modèle"], data["N° Série"]);
                break;
